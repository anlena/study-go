...
(function() {
    ...
    //为了在node.js环境下将underscore作为一个模块使用，并向后兼容旧版的模块API，即require。如果在浏览器环境中，则将underscore以_暴露到全局
    ...
    var optimizeCb = function(func, context, argCount) {
        ...
        switch (argCount == null ? 3 : argCount) { //如果传入了argCount,那么参数数量为argCount,如果传入等价为null,则为3，包括未传值的情况
            // 1个参数的时候，只需要传递当前值
            case 1:
    ...
    _.each = _.forEach = function(obj, iteratee, context) {
        ...
        if (isArrayLike(obj)) { //如果是类数组对象，则遍历每一个位置
            ...
            for (i = 0, length = keys.length; i < length; i++) {
                iteratee(obj[keys[i]], keys[i], obj); //参数值、键、完整集合
            }
        }
        return obj; //返回对象自身
    };
